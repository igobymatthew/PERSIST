...
class ComponentFactory:
    def __init__(self, config=None, config_path="config.yaml"):
        ...
        if config:
            self.config = validate_config(
                config=config, schema_path="schemas/config.schema.json"
            )
        else:
            self.config = load_and_validate_config(
                config_path=config_path, schema_path="schemas/config.schema.json"
            )
    ...
    def create_homeostat(self, meta_learner=None):
        ...
        initial_mu = (
            meta_learner.get_setpoints()
            if meta_learner
            else self.config["internal_state"]["mu"]
        )
        homeostat = Homeostat(mu=initial_mu, w=self.config["internal_state"]["w"])
    ...
    def create_internal_model(self, env):
        ...
        internal_model = InternalModel(
            internal_dim=env.internal_dim, act_dim=env.action_dim
        ).to(self.device)
    ...
    def create_near_boundary_buffer(self, env):
        ...
        buffer = NearBoundaryBuffer(
            state_dim=env.internal_dim, device=self.device, config=config
        )
        print("✅ Near-boundary buffer initialized.")
        return buffer

    def create_demonstration_buffer(self):
        if "demonstrations" in self.config and self.config["demonstrations"].get(
            "filepath"
        ):
            print("Initializing demonstration buffer...")
            buffer = DemonstrationBuffer(
                filepath=self.config["demonstrations"]["filepath"]
            )
            if len(buffer) == 0:
    ...
    def create_constraint_manager(self, env):
        ...
        if not (
            safety_config and chance_config and chance_config.get("enabled", False)
        ):
        ...
        manager = ConstraintManager(
            ...
            target_violation_rate=float(
                chance_config.get("target_violation_rate", 0.01)
            ),
    ...
    def create_ood_detector(self, viability_approximator):
        ...
        detector = OODDetector(
            model=viability_approximator, threshold=ood_config.get("threshold", -5.0)
        ).to(self.device)
    ...
    def create_safe_fallback_policy(self, env):
        ...
        policy = SafeFallbackPolicy(action_dim=env.action_dim, device=self.device)
    ...
    def _parse_constraints(self):
        ...
        for constr_str in constraints_config:
            ...
            match_interval = re.match(
                r"(\w+)\s+in\s+\[(-?[\d.]+),\s*(-?[\d.]+)\]", constr_str
            )
            if match_simple:
                ...
                if op == ">=":
                    parsed_constraints.append(
                        lambda x, _idx=idx, _val=val: _val - x[_idx]
                    )
                elif op == "<=":
                    parsed_constraints.append(
                        lambda x, _idx=idx, _val=val: x[_idx] - _val
                    )
            elif match_interval:
                ...
                idx = dim_map[var]
                parsed_constraints.append(
                    lambda x, _idx=idx, _val=min_val: _val - x[_idx]
                )
                parsed_constraints.append(
                    lambda x, _idx=idx, _val=max_val: x[_idx] - _val
                )
            else:
    ...
    def create_budget_meter(self):
        ...
        meter = BudgetMeter(budget_config=budget_config)
    ...
    def create_life_stage_manager(self):
        viability_cfg = self.config.get("viability", {})
    ...
    def create_ma_policies(self, env):
        ...
        if default_cfg["policy"] == "shared_sac":
            ...
            raise ValueError(
                f"Unsupported multi-agent policy type: {default_cfg['policy']}"
            )
    ...
    def get_all_components(self):
        ...
        if is_multi_agent:
            ...
            agent = self.create_agent(
                env, world_model, internal_model, viability_approximator
            )
            ...
            components = {
                ...
                "dynamics_adapter": dynamics_adapter,
                "cbf_layer": cbf_layer,
                "evaluator": evaluator,
                "device": self.device,
                "config": self.config,
