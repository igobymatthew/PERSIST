...
class Intrinsic: def init(self, mode="empowerment", **kwargs): ... def compute(self, s, a, o_next): ...
...
**Status:** Completed. `components/dreamer_world_model.py` introduces a Dreamer-style RSSM with imagination rollouts, value heads, and surprise rewards wired through the shield and MPC planners. Toggle it via `world_model.type: "dreamer"` in `config.yaml` and consult `tests/test_dreamer_world_model.py` for regression coverage.
...
**Status:** Completed via the `RND` module (`components/rnd.py`) with configuration wiring in `config.yaml`.
...
**Status:** Completed. The shield supports search and amortized modes backed by `components/safety_network.py`.
...
**Status:** Completed. Demonstration loading (`components/demonstration_buffer.py`) feeds pre-training routines in `components/viability_approximator.py` and the trainers.