...
class Trainer:
    ...
    def __init__(self, components):
        ...
        self.sequence_length = self.config.get("state_estimator", {}).get(
            "sequence_length", 16
        )
    ...
    def get_action(
        self, external_obs, obs_for_agent, state_for_components, total_steps
    ):
        """
        ...
        if self.ood_detector:
            state_tensor = torch.as_tensor(
                state_for_components, dtype=torch.float32, device=self.device
            ).unsqueeze(0)
        ...
        if is_ood:
            safe_action = (
                self.safe_fallback_policy.get_action().squeeze(0).cpu().numpy()
            )
            ...
            shielded_action = self.shield.project(
                state_for_components, unsafe_action, external_obs=external_obs
            )
            ...
            if self.cbf_layer:
                ...
                action_tensor = torch.as_tensor(
                    shielded_action, dtype=torch.float32, device=self.device
                )
                linearized_dynamics = self.dynamics_adapter.get_linearized_dynamics(
                    state_tensor, action_tensor
                )
                cbf_safe_action_tensor = self.cbf_layer(
                    action_tensor, state_tensor, linearized_dynamics
                )
    ...
    def calculate_intrinsic_reward(self, obs, act, next_obs):
        ...
        if intrinsic_method == "rnd":
            ...
            return self.intrinsic_reward_module.compute_intrinsic_reward(
                obs_tensor
            ).item()
            ...
            with torch.no_grad():
                ...
                action_sequence = (
                    torch.stack(
                        [
                            torch.from_numpy(self.env.action_space.sample())
                            for _ in range(k)
                        ]
                    )
                    .float()
                ...
                for i in range(k):
                    ...
                    current_z = self.world_model.transition(
                        current_z, action_for_rollout
                    )
                z_t_plus_k = current_z

                return self.intrinsic_reward_module.compute_reward(
                    z_t, action_sequence, z_t_plus_k
                ).item()
        else:  # 'surprise'
    ...
    def update_models(self):
        # 1. Sample batch
        seq_batch = self.replay_buffer.sample_sequence_batch(
        ...
        if self.is_partially_observable:
            act_seq_lagged = torch.cat(
                [torch.zeros_like(act_seq[:, :1]), act_seq[:, :-1]], dim=1
            )
            with torch.no_grad():
                estimated_internal_state_seq, _ = self.state_estimator(
                    obs_seq, act_seq_lagged
                )
            self.state_estimator.train_estimator(
                obs_seq, act_seq_lagged, true_internal_state_seq
            )
        else:
            estimated_internal_state_seq = true_internal_state_seq

        # 3. Update world model and intrinsic module
        if getattr(self.world_model, "is_dreamer", False):
            dreamer_batch = {
                "obs_seq": obs_seq,
                "action_seq": act_seq,
                "reward_seq": seq_batch["reward_seq"],
                "done_seq": seq_batch["done_seq"],
            }
            self.world_model.train_step(dreamer_batch)
        else:
            self.world_model.train_model(
                obs_seq[:, 0], act_seq[:, 0], next_obs_seq[:, 0]
            )
        intrinsic_method = self.config["rewards"]["intrinsic"]
        if intrinsic_method == "rnd":
            ...
            if getattr(self.world_model, "is_dreamer", False):
                raise RuntimeError(
                    "Empowerment intrinsic reward is not supported with the Dreamer world model."
                )
            with torch.no_grad():
                ...
                action_sequences_batch = (
                    torch.stack(
                        [
                            torch.from_numpy(
                                np.array(
                                    [self.env.action_space.sample() for _ in range(k)]
                                )
                            )
                ...
                for i in range(k):
                    ...
                    current_z_batch = self.world_model.transition(
                        current_z_batch, actions_for_rollout
                    )
        ...
        flat_est_internal = estimated_internal_state_seq.reshape(
            -1, self.env.internal_dim
        )
        flat_true_next_internal = true_next_internal_state_seq.reshape(
            -1, self.env.internal_dim
        )
        ...
        flat_constraint_margins = constraint_margins_seq.reshape(
            -1, self.env.num_constraints
        )

        self.internal_model.train_model(
            flat_est_internal, flat_act, flat_true_next_internal
        )

        # Train the safety probe
        if self.safety_probe:
            self.safety_probe.train_probe(
                flat_est_internal.detach(), flat_constraint_margins
            )

        if self.demonstration_buffer and len(self.demonstration_buffer) > 0:
            self.viability_approximator.train_on_demonstrations(
                self.demonstration_buffer, self.batch_size
            )
        ...
        if self.is_partially_observable:
            ...
            next_obs = torch.cat(
                [batch["next_obs"], batch["next_internal_state"]], dim=1
            )
