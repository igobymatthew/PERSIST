...
## CLI & TUI Experience
- **Adaptive step navigator**: Extend the `StepProgressTracker` so each step can expand with contextual help, example defaults, and quick doc links when users pause. Pair the navigator with inline mini-diffs that highlight how the current selection diverges from the base `config.yaml`, reinforcing why choices matter.
